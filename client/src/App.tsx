...
function Table (props: any) {
      ...
      if(!init){
        let interval = setInterval(async function() {
          ...
          if(nonce < 6*2 /* *round */){
            ...
            if(nonce % 2 == 0){
              nonce++
              // send particle to foe
              const res = await dealRound(localStorage.getItem('foe')!, card)
              console.log(res)
            } else {
              // send particle to hand
              nonce++
              setHand((prev: any) => [...prev, {...card, crib: false}])
              console.log(hand)
            }
          }else {
  ...
  const selected = (index: number) => {
    ...
    let count = 0;
    let pass = false
    const cribSum = hand.map((item: any, i: number) => {
      if(item.crib) {
        count = count + 1
        if(i == index) pass = true
      }
    })
  ...
  <>
    <main className='hand'>
      {hand.map((card: any, index: number) => {
        return <div onClick={() => selected(index)}>
          <Card key={index} suit={card.suit} denomination={card.denomination} pathColor={pathColor} backgroundColor={backgroundColor} cribSelect={card.crib}/>
          </div>
      })}
      {cards}
    </main>
...
function Card(props: any){
  ...
  <>
    <main>
        <section className={`${props.cribSelect ? 'card-raised' : ''}`} style={{border: `2px solid ${props.pathColor}`, background: `linear-gradient(to bottom, rgba(242, 61, 76, 0.2), ${props.backgroundColor} 66%)`}}>
          <aside style={{color: props.pathColor}}>
            <span>{props.denomination}</span>
            <span>{props.suit}</span>
          </aside>
          <p style={{color: props.pathColor}} className="card">{props.denomination}</p>
          <aside style={{color: props.pathColor}}>
            <span>{props.denomination}</span>
            <span>{props.suit}</span>
          </aside>
...
function App() {
  ...
  const [appState, setAppState] = React.useState(6)
